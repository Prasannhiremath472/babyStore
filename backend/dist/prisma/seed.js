...
async function main() {
    ...
    const superAdminHash = await bcryptjs_1.default.hash('SuperAdmin@123', 12);
    const adminHash = await bcryptjs_1.default.hash('Admin@123', 12);
    const customerHash = await bcryptjs_1.default.hash('Customer@123', 12);
    await prisma.user.upsert({
        ...
        create: {
            ...
            role: client_1.UserRole.SUPER_ADMIN, status: client_1.UserStatus.ACTIVE, emailVerified: true,
    ...
    await prisma.user.upsert({
        ...
        create: {
            ...
            role: client_1.UserRole.ADMIN, status: client_1.UserStatus.ACTIVE, emailVerified: true,
            wallet: { create: {} }, cart: { create: {} },
        },
    });
    await prisma.user.upsert({
        ...
        create: {
            ...
            role: client_1.UserRole.MANAGER, status: client_1.UserStatus.ACTIVE, emailVerified: true,
            wallet: { create: {} }, cart: { create: {} },
        },
    });
    const customersData = [
    ...
    for (const c of customersData) {
        const user = await prisma.user.upsert({
            ...
            create: {
                ...
                role: client_1.UserRole.CUSTOMER, status: client_1.UserStatus.ACTIVE, emailVerified: true,
    ...
    for (let pi = 0; pi < Math.min(createdProducts.length, 25); pi++) {
        ...
        for (let ri = 0; ri < numReviews; ri++) {
            ...
            if (usedCustomers.has(customer.id))
                continue;
    ...
    const paymentMethods = [client_1.PaymentMethod.RAZORPAY, client_1.PaymentMethod.COD, client_1.PaymentMethod.UPI];
    let orderCount = 0;
        ...
        for (let oi = 0; oi < numOrders; oi++) {
            ...
            const order = await prisma.order.create({
                data: {
                    ...
                    createdAt,
                    updatedAt: createdAt,
                    deliveredAt: status === client_1.OrderStatus.DELIVERED ? new Date(createdAt.getTime() + 5 * 86400000) : undefined,
                    items: { create: items },
                    timeline: {
                        create: [
            ...
            await prisma.payment.create({
                data: {
                    orderId: order.id,
                    method,
                    status: status === client_1.OrderStatus.PENDING ? client_1.PaymentStatus.PENDING : client_1.PaymentStatus.CAPTURED,
                    amount: totalAmount,
                    currency: 'INR',
                    razorpayOrderId: method === client_1.PaymentMethod.RAZORPAY ? `order_${Math.random().toString(36).substring(2, 15)}` : undefined,
                    razorpayPaymentId: method === client_1.PaymentMethod.RAZORPAY && status !== client_1.OrderStatus.PENDING ? `pay_${Math.random().toString(36).substring(2, 15)}` : undefined,
                },
            });
            orderCount++;
        }
    ...
    const bannersData = [
        { title: 'Welcome to LittleNest', subtitle: 'Premium Baby & Kids Products', image: 'https://images.unsplash.com/photo-1519689680058-324335c77eba?w=1400&q=90', link: '/products', type: client_1.BannerType.HERO, sortOrder: 1 },
        { title: 'Up to 50% Off on Toys', subtitle: 'Shop the best toys for your little ones', image: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=1400&q=90', link: '/category/toys-games', type: client_1.BannerType.HERO, sortOrder: 2 },
        { title: 'New Arrivals: Baby Clothing', subtitle: 'Organic & comfortable', image: 'https://images.unsplash.com/photo-1522771930-78848d9293e8?w=1400&q=90', link: '/products?isNew=true', type: client_1.BannerType.HERO, sortOrder: 3 },
        { title: 'Nursery Collection', subtitle: 'Create the perfect space', image: 'https://images.unsplash.com/photo-1555252333-9f8e92e65df9?w=1400&q=90', link: '/category/nursery', type: client_1.BannerType.PROMOTIONAL, sortOrder: 1 },
        { title: 'Back to School Sale', subtitle: 'Backpacks, stationery & more', image: 'https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=1400&q=90', link: '/category/school-essentials', type: client_1.BannerType.PROMOTIONAL, sortOrder: 2 },
        { title: 'Free Delivery Above ₹499', subtitle: 'Shop now and save on shipping', image: 'https://images.unsplash.com/photo-1519689680058-324335c77eba?w=800&q=90', link: '/products', type: client_1.BannerType.SIDEBAR, sortOrder: 1 },
    ];
    ...
    for (const c of couponsData) {
        await prisma.coupon.upsert({
            ...
            create: {
                ...
                discountType: c.type, discountValue: c.value,
    ...
    for (const b of blogsData) {
        ...
        await prisma.blog.upsert({
            ...
            create: {
                ...
                authorId: (await prisma.user.findFirst({ where: { role: client_1.UserRole.ADMIN } })).id,
